...
async def get_live_aqi():
    """Fetch real-time AQ data from OpenAQ."""
    return await openaq_service.get_latest_data()

@router.get("/aqi/grid")
async def get_aqi_grid():
    """Get the predicted AQI for the 250m x 250m grid."""
    return await prediction_service.get_full_grid()

@router.get("/aqi/hotspots")
async def get_hotspots():
    """Identify pollution hotspots and corridors."""
    return await hotspot_service.get_hotspots()
...
async def get_location_aqi(
    ...
    try:
        ...
        return {
            ...
            "aqi_category": _get_aqi_category(aqi_data.get("aqi", 0)),
...
async def get_analytics_summary(
    ...
    return {
        ...
        "category_distribution": _get_category_distribution(aqi_values)
    }

def _get_category_distribution(aqi_values: List[float]) -> dict:
